<template>
  <div class="upload-cover">
    <el-upload
      ...
      @change="fileChange"
    >
      <img v-if="currentCover && !uploading" :src="getResourceUrl(currentCover)" class="cover" alt="封面" />
      ...
      <div class="upload-tips" v-else>
        ...
        <p class="upload-limit">
          上传文件大小需小于{{ globalConfig.maxImgSize }}M，支持 .jpg .jpeg .png .gif 格式文件
        </p>
      </div>
    </el-upload>
...
const beforeUploadCover = async (file: any) => {
  const isValidType = ["image/jpeg", "image/png", "image/gif"].includes(file.type); // 支持 .gif 格式
  if (!isValidType) {
    ElMessage.error("文件只支持 jpg/jpeg/png/gif 格式");
    return false; // 阻止上传
  }

  const isLtMaxSize = file.size / 1024 / 1024 < globalConfig.maxImgSize;
  if (!isLtMaxSize) {
    ElMessage.error(`图片大小不能超过 ${globalConfig.maxImgSize}MB`);
    return false; // 阻止上传
  }
